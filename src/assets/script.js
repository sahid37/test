...
const storiesLeftButton = document.querySelector('.stories-left-button');
const storiesRightButton = document.querySelector('.stories-right-button');
...
posts.forEach((post) => {
  if (post.querySelectorAll('.post-media').length > 1) {
    ...
    leftButtonElement.classList.add('post-left-button');
    ...
    rightButtonElement.classList.add('post-right-button');
    ...
    post.querySelectorAll('.post-media').forEach(function () {
      ...
      postMediaIndicatorElement.classList.add('post-indicator');

      post
        .querySelector('.post-indicators')
        .appendChild(postMediaIndicatorElement);
    });

    // Observer to change the actual media indicator
    const postMediasContainer = post.querySelector('.post-medias');
    const postMediaIndicators = post.querySelectorAll('.post-indicator');
    ...
    const postMedias = post.querySelectorAll('.post-media');
...
postsContent.forEach((post) => {
  if (post.querySelectorAll('.post-media').length > 1) {
    const leftButton = post.querySelector('.post-left-button');
    const rightButton = post.querySelector('.post-right-button');
    const postMediasContainer = post.querySelector('.post-medias');
    ...
    const postButtonObserver = new IntersectionObserver(
      function (entries) {
        entries.forEach((entry) => {
          if (entry.target === post.querySelector('.post-media:first-child')) {
            leftButton.style.display = entry.isIntersecting ? 'none' : 'unset';
          } else if (
            entry.target === post.querySelector('.post-media:last-child')
          ) {
    ...
    if (window.matchMedia('(min-width: 1024px)').matches) {
      postButtonObserver.observe(
        post.querySelector('.post-media:first-child')
      );
      postButtonObserver.observe(post.querySelector('.post-media:last-child'));
    }
  }
});
